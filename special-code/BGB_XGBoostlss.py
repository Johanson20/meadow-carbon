...
# drop null rows of relevant columns
nullIds = list(np.where(subdata[y_field].isnull())[0])
data.drop(nullIds, inplace = True)
data.dropna(subset=[y_field], inplace=True)
data.dropna(subset=var_col, inplace=True)
data.reset_index(drop=True, inplace=True)

# split model into 80/20 for training/test (rows of same are exclusively in training/test set)
...
# convert training/test datasets to matrix format suitable for training in xgblss
...
# predict trained model on test and training data
